...
use components::{team, is_creep, creep_current_state, creep_next_state, pursuit_target, attack_target, projectile_speed_vector};
    ...
    query((components::is_creep(), attack_target())).each_frame({
        move |list| {
            for (creep_model, (_, target_entity)) in list {
                let projectile_direction = 
                    entity::get_component(target_entity, translation()).unwrap() -
                    entity::get_component(creep_model, translation()).unwrap();
                let projectile_direction = 
                    projectile_direction.normalize();
                create_pursuing_projectile(projectile_direction * CREEP_PROJECTILE_SPEED,
...
fn create_pursuing_projectile(proj_speed_vector: Vec3, cube_collider_size: Vec3, proj_scale: Vec3, proj_translation: Vec3, proj_color: Vec4) -> EntityId {
    ...
    Entity::new()
                            ...
                            .with(cast_shadows(), ())
                            .with(speed(), "add speed value here")
                            .with(cube_collider(), "add cube size here?")
                            .with(dynamic(), true)
                            .with(scale(), "add scale here")
